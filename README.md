...
もっとも成績が悪い局面を、直近の棋譜から再現して打ち直すモードです。`詰碁`は死活の問題を解くモードです
(全290問。20級〜9段の段級位ごとに10問ずつで、難易度は正解手順の長さ(黒の手数)で決めています。
目安は1手詰め=20級・2手詰め=15級・3手詰め=10級・4手詰め=5級・5手詰め以上=段位です。
囲いの中に黒石・白石が入った攻め合いや捨て石の問題も含みます)。操作方法・
勝率表示・待った・棋譜の保存と再生・次の一手クイズ・レーティング・AIの強さ設定・苦手分野の解析など各機能の詳細は
...
KurenaiGo/
  ...
  Assets/
    ...
    Tsumego/                 詰碁の問題集(tsumego.sgf。1ファイルに全290問。SGFファイルの追加も可能)
  Tools/
