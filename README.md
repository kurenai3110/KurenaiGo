...
自動チューニングで数十秒かかることがあります)、起動直後に「レート戦として対局しますか?」
という確認ダイアログが表示されます。「はい」を選ぶとレート戦(対局結果がレーティングに反映
される)、「いいえ」を選ぶとカジュアル(何度でも自由に打ち直せる、レーティングに影響しない
対局)になります。この確認はKataGoの起動完了を待たずに表示されるため、待ち時間が増えることは
ありません。準備ができ次第、黒番(人間)から対局が始まります。
...
アプリを再起動する必要はなく、何度でも打ち直せます。選ぶとレート戦/カジュアルの確認ダイアログが
再度表示され、盤面・アゲハマ・着手履歴がすべてリセットされたうえで黒番(人間)から対局が始まります。

盤の下には現在の手番状態・アゲハマ(お互いが取った石の数)・レーティング(棋力の数値化)と
現在の対局モード([レート戦]/[カジュアル])を表示するHUDが常時表示されます。レーティングは
標準的なElo式で計算され、初期値は1500です。レート戦で勝つと上がり、負けると下がります
(カジュアル対局は影響しません)。KataGo自体の強さを調整する機能はまだ無く、常に一定の強さの
相手として扱っているため、このレーティングは実世界の段級位やKataGoの実力を表す値ではなく、
「この固定強度の相手に対する、あなた自身の対局結果の推移」を数値化したものです。レート戦の
結果は`rating_history.txt`(実行ファイルと同じ場所)に1局ごと追記され、次回起動時にも
引き継がれます。
