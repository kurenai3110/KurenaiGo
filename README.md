...
- [x] Human SLモデルによるアマチュア向けの人間らしい打ち筋の再現(20級〜9段、任意配置)
...
(`-human-model`起動引数、`humanSLProfile`設定)が使える場合、実際の段級位(20級〜9段)に
応じた人間の棋譜の打ち筋(悪手も含む)を再現します。通常のニューラルネットは探索回数を絞っても
一手ごとの質が高いままアマチュアには強すぎるため、この方式でなければ「弱いAI」を体感しにくいという
課題に対応したものです。Human SLモデル(`b18c384nbt-humanv0.bin.gz`)が未配置の場合は、
従来どおり探索回数(`maxVisits`)だけで強さを調整する方式にフォールバックします(下記セットアップ
「5. Human SLモデルの配置」参照)。
...
AIの強さをアマチュア向けの実際の段級位(20級〜9段)で再現するには、KataGo公式の
「Human SLモデル」を追加で配置します。
...
このファイルは任意です。配置しない場合は、従来どおり探索回数(`maxVisits`)だけで強さを
調整する方式にフォールバックし、動作は継続します。`-human-model`起動引数はKataGo v1.15.0以降で
使えます(本リポジトリで動作確認しているv1.16.5では利用できます)。
